...
class BootloaderPlugin {
    ...
    processHtmlAsset(publicPath, src, assets, excludeFiles, result) {
        ...
        if (excludeFiles.has(scriptName)) return;
        const asset = assets[scriptName];
        if (!asset) return;
        result.push({
            file: src,
    ...
    apply(compiler) {
        ...
        compiler.hooks.entryOption.tap('BootloaderPlugin', (context) => {
            compiler.hooks.make.tapAsync('BootloaderPlugin', (compilation, callback) => {
                const entry = SingleEntryPlugin.createDependency(this.options.script, this.options.name);
        ...
        compiler.hooks.thisCompilation.tap('BootloaderPlugin', (compilation) => {
            ...
            hooks.beforeAssetTagGeneration.tap('BootloaderPlugin', ({assets}) => {
                ...
                if (entrypoint) {
                    ...
                    assets.js.forEach(src => this.processHtmlAsset(assets.publicPath, src, compilation.assets, bootloaderFiles, htmlAssets.js));
                    assets.css.forEach(src => this.processHtmlAsset(assets.publicPath, src, compilation.assets, bootloaderFiles, htmlAssets.css));
                }
            });
            hooks.alterAssetTags.tap('BootloaderPlugin', ({assetTags}) => {
                const entrypoint = compilation.entrypoints.get(this.options.name);
                if (entrypoint) {
                    const bootloaderFiles = new Set(entrypoint.getFiles().map(filename => publicPath + filename));
                    assetTags.scripts = assetTags.scripts
                        .filter(tag => this.isBootloaderScriptTag(tag, bootloaderFiles))
                        .map(tag => this.inlineScriptTag(publicPath, compilation.assets, tag));
                    assetTags.styles = assetTags.styles
                        .filter(tag => this.isBootloaderStyleTag(tag, bootloaderFiles))
                        .map(tag => this.inlineStyleTag(publicPath, compilation.assets, tag));
                    // inject assets
                    const assetSource = `!function(){var bootloader={};bootloader.assets=${JSON.stringify(htmlAssets)};window.$bootloader=bootloader;}();`;
                    assetTags.scripts.unshift({tagName: 'script', innerHTML: assetSource, closeTag: true});
                    // removing bootloader files from assets so webpack will not emit them
                    entrypoint.getFiles().forEach(filename => delete compilation.assets[filename]);
                }
            });
        });
    }
